...
int main(){
  ...
  copy1.tab = (int**) malloc(sizeof(int*)*BOARDSIZE);  
  copy2.tab = (int**) malloc(sizeof(int*)*BOARDSIZE);  
  
  for (int i = 0; i < BOARDSIZE; i++) {
    copy1.tab[i] = (int*) malloc(sizeof(int)*BOARDSIZE);  
    copy2.tab[i]= (int*) malloc(sizeof(int)*BOARDSIZE);  
  }
  copy_board(copy1.tab,Board->grid,BOARDSIZE);
  copy_board(copy2.tab,Board->grid,BOARDSIZE);
  int turn = BLACK;
  ...
  while(fgets(move,7,stdin)){
    list_possible_moves(x,y,&n,Board,&copy1,&copy2,turn,count,Ko_check);
    printf("The available moves are: \n");
    printf("%d",n);
    for(int j = 0; j < n;j++) {printf("x: %d, y: %d\n",x[j],y[j]);}
    n = 0;
    int i = play_move_out_of_match(Board,&copy1,&copy2,&turn,&pass_count,&count,move,&Ko_check);
    if (i == 1){
      printf("Game is over\n");
      break;
    }
